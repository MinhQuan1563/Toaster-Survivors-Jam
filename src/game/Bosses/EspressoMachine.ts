...
export class EspressoMachine extends BaseEnemy {
  ...
  update(time: number, delta: number) {
    ...
    if (this.stateEspresso !== "DASHING") {
      ...
      this.visual.scaleX = Phaser.Math.Linear(
        this.visual.scaleX,
        isFacingLeft ? -1 : 1,
        0.2,
      );
    ...
    if (this.stateEspresso === "JITTER") {
      ...
      this.dashTargetAngle = Phaser.Math.Angle.Between(
        this.x,
        this.y,
        player.x,
        player.y,
      ...
      if (time > this.lastPuddleTime + 80) {
        ...
        this.spawnSteam(
          0,
          30,
          -Math.cos(this.dashTargetAngle) * 5,
          -Math.sin(this.dashTargetAngle) * 5,
        );
      }
      ...
      this.visual.x = 0;
      this.visual.y = 0;
      this.gaugeNeedleAngle = Phaser.Math.Linear(
        this.gaugeNeedleAngle,
        0,
        0.05,
      );
  ...
  private burstCoffeeBeans() {
    for (let i = 0; i < 8; i++) {
      const bean = this.beansGroup.create(
        this.x,
        this.y - 10,
        "coffee_bean",
      ...
      bean.setData(
        "rotSpeed",
        Phaser.Math.FloatBetween(15, 30) * Phaser.Math.RND.sign(),
      );
      this.sceneRef.time.delayedCall(2000, () => {
        if (bean.active) bean.destroy();
      });
    }
  ...
  private drawLocalHpBar() {
    ...
    this.hpBarGraphics.fillRoundedRect(
      bx - 3,
      by - 3,
      barWidth + 6,
      barHeight + 6,
  ...
  protected drawEnemy() {
    ...
    g.fillRoundedRect(-w / 2, -h / 2, w - 6, h - 4, {
      tl: 8,
      tr: 4,
      bl: 8,
      br: 4,
    ...
    g.beginPath();
    g.moveTo(-w / 2 + 6, -h / 2 - 6);
    g.lineTo(w / 2 - 6, -h / 2 - 6);
    g.strokePath();
    ...
    if (this.engineGlow > 0) {
      g.fillStyle(
        0xff0000,
        this.engineGlow * Math.abs(Math.sin(this.animFrame * 0.2)),
      );
      g.fillCircle(12, 5, 4);
    }
  }
