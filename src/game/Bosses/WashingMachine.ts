...
export class WashingMachine extends BaseEnemy {
    ...
    constructor(scene: GameScene, x: number, y: number, hp: number, speed: number, damage: number) {
        ...
        scene.physics.add.overlap(scene.player, this.socksGroup, (player: any, sock: any) => {
            if (sock.active) {
    ...
    update(time: number, delta: number) {
        ...
        const spinSpeed = this.stateWashingMachine === 'SPINNING' ? 25 : (this.stateWashingMachine === 'CHASE' ? 2 : 0);
        ...
        if (this.stateWashingMachine === 'CHASE') {
            ...
            if (time > this.nextSockTime && this.socksFired < 6) {
                ...
                this.socksFired++;
                this.sceneRef.tweens.add({ targets: this.visual, x: -10 * this.visual.scaleX, yoyo: true, duration: 100 });
            }
    ...
    private switchState(newState: 'CHASE' | 'SHOOTING' | 'SPINNING') {
        ...
        this.sceneRef.tweens.add({ targets: this.visual, y: -15, yoyo: true, duration: 150 });
    }

    private fireBoomerangSock() {
        const sock = this.socksGroup.create(this.x, this.y, 'dirty_sock') as Phaser.Physics.Arcade.Sprite;
        const angle = Phaser.Math.Angle.Between(this.x, this.y, this.sceneRef.player.x, this.sceneRef.player.y);
        ...
        sock.setData('rotSpeed', Phaser.Math.FloatBetween(5, 15) * Phaser.Math.RND.sign());
    }
    ...
    private updateSocks(dt: number) {
        this.socksGroup.getChildren().forEach((s: any) => {
            ...
            if (life > 0.5) {
                const angleToBoss = Phaser.Math.Angle.Between(sock.x, sock.y, this.x, this.y);
                const returnAccel = 800; 
                sock.setAcceleration(Math.cos(angleToBoss) * returnAccel, Math.sin(angleToBoss) * returnAccel);
                const distToBoss = Phaser.Math.Distance.Between(sock.x, sock.y, this.x, this.y);
    ...
    private applySuctionForce(player: any, dt: number, time: number) {
        ...
        for(let i=0; i<3; i++) {
            ...
            this.vortexGraphics.lineStyle(4, 0x94a3b8, 0.1 + (1 - radius/400) * 0.4);
    ...
    protected drawEnemy() {
        ...
        g.beginPath(); g.moveTo(-w/2, -h/2 + 20); g.lineTo(w/2, -h/2 + 20); g.strokePath();
        ...
        if (this.angryGlow > 0) {
            ...
            g.rotateCanvas(0.3); g.fillRect(-30, 0, 20, 6);
            g.rotateCanvas(-0.6); g.fillRect(10, 0, 20, 6);
            g.restore();
        }
        ...
        g.lineStyle(6, 0xDD6B20); g.beginPath(); g.arc(0, 0, 14, 0, Math.PI * 0.5, false); g.strokePath();
        g.lineStyle(8, 0xE2E8F0); g.beginPath(); g.arc(0, 0, 20, Math.PI, Math.PI * 1.5, false); g.strokePath();
        g.lineStyle(4, 0x48BB78); g.beginPath(); g.arc(0, 0, 10, Math.PI*0.7, Math.PI*1.2, false); g.strokePath();
        g.restore();
        ...
        if (isSpinning && Math.random() < 0.3) {
            this.foamBubbles.push({x: Phaser.Math.Between(-20, 20), y: 35, life: 1});
        }
