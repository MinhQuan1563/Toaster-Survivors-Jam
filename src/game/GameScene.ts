...
export default class GameScene extends Phaser.Scene {
  ...
  update(_time: number, delta: number) {
    ...
    if (this.weaponTimer >= currentCooldown && !this.isFrozen && !this.isFrozenByTrap) {
      this.weaponTimer = 0;
      this.fireToast();
    }
  ...
  private triggerQuizInteraction() {
    ...
    if (closestFridge && closestFridge.canBeUsed()) {
      ...
      if (closestFridge && !closestFridge.canBeUsed()) console.log("Fridge on cooldown");
    }
  }
  ...
  private drawQuizUI() {
    ...
    questionLines.forEach((line, idx) => {
      const text = this.add.text(centerX, panelY + 40 + idx * 20, line, {
        fontFamily: "monospace",
        fontSize: "16px",
        color: "#ffffff",
        align: "center",
    ...
    const tA = this.add.text(btnX + btnW / 2, aY + btnH / 2,
    ...
    const tB = this.add.text(btnX + btnW / 2, bY + btnH / 2,
  ...
  spawnTrap() {
    ...
    const clampedX = Phaser.Math.Clamp(
      x,
      halfW,
      GAME_CONFIG.WORLD_W - halfW
    );
    const clampedY = Phaser.Math.Clamp(
      y,
      halfH,
      GAME_CONFIG.WORLD_H - halfH
    );
  ...
  private drawFrozenBlockEffect(timeMs: number) {
    ...
    g.beginPath();
    g.moveTo(x + w * 0.52, y + h * 0.30);
    g.lineTo(x + w * 0.70, y + h * (0.18 + 0.02 * shimmer));
    g.lineTo(x + w * 0.82, y + h * 0.32);
    g.moveTo(x + w * 0.52, y + h * 0.30);
    g.lineTo(x + w * 0.40, y + h * 0.52);
