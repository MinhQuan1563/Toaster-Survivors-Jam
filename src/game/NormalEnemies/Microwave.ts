...
export class Microwave extends BaseEnemy {
    ...
    private drawWarningCircle(time: number, progress: number) {
        ...
        this.warningGraphics.lineStyle(2 + progress * 2, 0xff0000, 0.5 + progress * 0.5);
    ...
    private spawnBeepText(progress: number) {
        ...
        const text = this.sceneRef.add.text(this.x, this.y - 35, 'BEEP!', {
            fontSize: `${size}px`,
    ...
    protected drawEnemy() {
        ...
        const progress = isPrimed ? 1 - Math.max(0, this.explodeTimer / this.maxTimer) : 0;
        ...
        if (isPrimed) {
            ...
            if (Math.floor(this.sceneRef.time.now / (100 - progress * 50)) % 2 === 0) {
