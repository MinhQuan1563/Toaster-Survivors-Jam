...
export class Roomba extends BaseEnemy {
    ...
    update(time: number, delta: number) {
        ...
        if (!this.targetOrb || !this.targetOrb.active) {
            ...
            this.sceneRef.orbs.getChildren().forEach((o: any) => {
                ...
                if (d < near) { near = d; this.targetOrb = o; }
            });
        }
        
        if (this.targetOrb) {
            // Rush towards XP
            this.sceneRef.physics.moveToObject(this, this.targetOrb, this.speed * 1.5);
            
            // Absorb XP if close enough
            const dist = Phaser.Math.Distance.Between(this.x, this.y, this.targetOrb.x, this.targetOrb.y);
            if (dist < 25 && this.targetOrb.active) {
                ...
                // Expand when eating
                this.sceneRef.tweens.add({ targets: this.visual, scale: 1.3, yoyo: true, duration: 150 });
            }
        } else {
    ...
    protected drawEnemy() {
        ...
        for(let i=0; i<3; i++) {
            ...
            g.lineBetween(-20, -15, -20 + Math.cos(angle)*18, -15 + Math.sin(angle)*18);
        }
        for(let i=0; i<3; i++) {
            const angle = -spinAngle + (i * Math.PI * 2 / 3);
            g.lineBetween(20, -15, 20 + Math.cos(angle)*18, -15 + Math.sin(angle)*18);
